...
_INDEX_KEYWORDS = [
    "指数", "ETF联接", "沪深300", "中证500", "中证800", "中证1000",
    "上证50", "上证180", "创业板", "纳指", "纳斯达克", "标普",
    "MSCI", "恒生", "科创50", "北证50", "双创", "科创板",
    "中证100", "中证200", "红利", "价值", "成长指数",
]

# 主动管理特征词 —— 即使含指数关键词也排除
_ACTIVE_KEYWORDS = ["主动", "精选灵活", "量化精选"]


def _is_index_fund(name: str) -> bool:
    """判断是否为指数型基金（估算净值可信）"""
    name = str(name)
    if any(kw in name for kw in _ACTIVE_KEYWORDS):
        return False
...
def detect(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 1. 只保留指数型 LOF（估算净值可信）
    if '名称' in df.columns:
        index_mask = df['名称'].apply(_is_index_fund)
